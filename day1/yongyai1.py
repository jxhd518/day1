...
from dao.heart.engine import CommonDao
from db import session_close
from models.heart import PatientFollowUpFormData, Patient, PatientLabelRecord, OrganizePatientRel, Organize
import xlwt
import re
...
def export_patients():
    ...
    for patient in patients:
        ...
        medics_data = db_handle.fetch_list(PatientFollowUpFormData,
                                           ...
                                           data_status=2)
#        if medics_data is None:
        if len(medics_data) == 0:
    ...
    for item in target_list:
        row_number += 1
        table.write(row_number, 0, item['patient_id'])
        table.write(row_number, 1, item['real_name'])
        table.write(row_number, 2, item['mobile'])
        table.write(row_number, 3, item['created'])
#        table.write(row_number, 4, item['med'])
    filename = '数据'
